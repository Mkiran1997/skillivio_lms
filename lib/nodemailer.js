...
const SMTP_PORT = Number(process.env.SKILLIVIO_SMTP_PORT || 587);

const transporter = nodemailer.createTransport({
  host: process.env.SKILLIVIO_SMTP_HOST,
  port: process.env.SKILLIVIO_SMTP_PORT,
  secure: SMTP_PORT === 465,
  auth: {
    user: process.env.SKILLIVIO_SMTP_USER,
    pass: process.env.SKILLIVIO_SMTP_PASS,
  },
});

function assertSmtpConfig() {
  const required = {
    SKILLIVIO_SMTP_HOST: process.env.SKILLIVIO_SMTP_HOST,
    SKILLIVIO_SMTP_USER: process.env.SKILLIVIO_SMTP_USER,
    SKILLIVIO_SMTP_PASS: process.env.SKILLIVIO_SMTP_PASS,
    SKILLIVIO_EMAIL_TO: process.env.SKILLIVIO_EMAIL_TO,
  };
...
export async function sendContactUsEmail(formData) {
  ...
  const fromEmail = `"Skillivio Notifications" <${process.env.SKILLIVIO_SMTP_USER}>`;
  const toEmail = process.env.SKILLIVIO_EMAIL_TO;
